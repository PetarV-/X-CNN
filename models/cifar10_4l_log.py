...
(X_train, Y_train), (X_test, Y_test) = get_cifar(p=train_ratio, append_test=False, use_c10=True)
...
if not data_augmentation:
    ...
    history_log=model.fit_generator(datagen.flow(X_train, Y_train,
                        batch_size=batch_size),
    ...
    csv_filename='cifar10_4l_'+str(train_ratio)+'.csv'
    val_acc=np.array(history_log.history['val_acc'])
    np.savetxt(csv_filename,val_acc)
